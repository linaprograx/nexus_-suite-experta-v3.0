...
export const TaskDetailModal: React.FC<TaskDetailModalProps> = ({ task, onClose, db, userId, appId, auth, storage, onAnalyze, enabledTools = [] }) => {
    ...
    return (
        <Modal isOpen={true} onClose={onClose} size="3xl">
            <div className="flex h-[85vh] bg-white/95 dark:bg-slate-900/95 backdrop-blur-xl overflow-hidden rounded-lg border border-white/20 dark:border-slate-700/50">

                {/* LEFT COLUMN: ACTIVITY / CHAT (Fixed 300px) */}
                <div className="w-[300px] min-w-[300px] border-r border-slate-100/50 dark:border-slate-800/50 flex flex-col bg-slate-50/50 dark:bg-slate-900/30">
                    ...
                    <div className="p-4 border-t border-slate-200/50 dark:border-slate-700/50 bg-white/50 dark:bg-slate-900/50 backdrop-blur-sm">
                        <textarea
                            ...
                            className="w-full text-sm bg-slate-50/50 dark:bg-slate-800/50 border-slate-200/50 dark:border-slate-700/50 rounded-lg p-3 min-h-[80px] focus:ring-2 focus:ring-nexus-orange/20 outline-none resize-none mb-2 backdrop-blur-sm"
                            placeholder="Escribe un comentario..."
                        />
                        <div className="flex justify-end">
                            <Button size="sm" onClick={handleSendComment} className="bg-nexus-orange text-white hover:bg-orange-600 shadow-sm">Enviar</Button>
                        </div>
                    </div>
                </div>
                ...
                <div className="flex-1 flex flex-col h-full relative overflow-hidden bg-white/60 dark:bg-slate-900/60 backdrop-blur-sm">

                    {/* HEADER */}
                    <div className="p-4 border-b border-slate-100/50 dark:border-slate-800/50 flex justify-between items-center bg-white/50 dark:bg-slate-900/50 z-10 backdrop-blur-md">
                        ...
                        <div className={activeTool !== 'details' ? 'opacity-50 pointer-events-none filter blur-[1px] transition-all' : 'transition-all'}>
                            ...
                            <div className="mb-6">
                                ...
                                <Textarea
                                    ...
                                    className="w-full min-h-[100px] bg-slate-50/50 dark:bg-slate-900/30 border-transparent rounded-xl p-4 text-sm text-slate-600 focus:bg-white/80 focus:ring-2 focus:ring-nexus-orange/20 transition-all resize-none backdrop-blur-sm"
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                                <div className="bg-white/50 dark:bg-slate-800/50 p-3 rounded-xl border border-gray-100 dark:border-slate-700 shadow-sm backdrop-blur-sm">
                            ...
                            {/* TAGS */}
                            <div className="mb-6 bg-white/50 dark:bg-slate-800/50 p-4 rounded-xl border border-gray-100 dark:border-slate-700 shadow-sm backdrop-blur-sm">
                                <span className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-2 block">Etiquetas</span>
                                <div className="flex flex-wrap gap-2">
                                    {labels.map(label => (
                                        <span key={label} className="px-2 py-1 bg-gray-100/50 dark:bg-slate-700/50 text-gray-600 dark:text-gray-300 text-xs rounded-md flex items-center gap-1 border border-slate-200/50">
                                            #{label}
                                            <button onClick={() => handleLabel('remove', label)} className="hover:text-red-500"><Icon svg={ICONS.x} className="w-3 h-3" /></button>
                                        </span>
                                    ))}
                                    <div className="flex gap-2">
                                        <Input value={newLabel} onChange={e => setNewLabel(e.target.value)} placeholder="Añadir..." className="h-7 text-xs w-24 bg-white/50" />
                                ...
                                {enabledTools.includes('grimorium') && (
                                    <div className="mb-4">
                                        ...
                                        <div className="grid grid-cols-4 gap-2">
                                            <button onClick={() => setActiveTool('grimorium_recipes')} className="flex flex-col items-center justify-center p-3 rounded-lg bg-emerald-50/50 hover:bg-emerald-100 text-emerald-700 transition-colors gap-2 hover:scale-105 transform duration-200 border border-emerald-100/50">
                                                <Icon svg={ICONS.book} className="w-5 h-5" /> <span className="text-xs font-medium">Recetas</span>
                                            </button>
                                            <button onClick={() => setActiveTool('grimorium_ingredients')} className="flex flex-col items-center justify-center p-3 rounded-lg bg-lime-50/50 hover:bg-lime-100 text-lime-700 transition-colors gap-2 hover:scale-105 transform duration-200 border border-lime-100/50">
                                                <Icon svg={ICONS.leaf} className="w-5 h-5" /> <span className="text-xs font-medium">Ingredientes</span>
                                            </button>
                                            <button onClick={() => setActiveTool('grimorium_costing')} className="flex flex-col items-center justify-center p-3 rounded-lg bg-amber-50/50 hover:bg-amber-100 text-amber-700 transition-colors gap-2 hover:scale-105 transform duration-200 border border-amber-100/50">
                                                <Icon svg={ICONS.dollarSign} className="w-5 h-5" /> <span className="text-xs font-medium">Escandallo</span>
                                            </button>
                                            <button onClick={() => setActiveTool('grimorium_zerowaste')} className="flex flex-col items-center justify-center p-3 rounded-lg bg-teal-50/50 hover:bg-teal-100 text-teal-700 transition-colors gap-2 hover:scale-105 transform duration-200 border border-teal-100/50">
                                ...
                                {enabledTools.includes('cerebrity') && (
                                    <div className="mb-4">
                                        ...
                                        <div className="grid grid-cols-1 gap-2">
                                            <button onClick={() => setActiveTool('cerebrity')} className="flex items-center justify-center p-3 rounded-lg bg-violet-50/50 hover:bg-violet-100 text-violet-700 transition-colors gap-2 hover:scale-[1.02] transform duration-200 border border-violet-100/50">
                                    ...
                                    <div className="mb-4">
                                        ...
                                        <div className="grid grid-cols-4 gap-2">
                                            {enabledTools.includes('batcher') && (
                                                <button onClick={() => setActiveTool('batcher')} className="flex flex-col items-center justify-center p-3 rounded-lg bg-blue-50/50 hover:bg-blue-100 text-blue-700 transition-colors gap-2 hover:scale-105 transform duration-200 border border-blue-100/50">
                                                    <Icon svg={ICONS.layers} className="w-5 h-5" /> <span className="text-xs font-medium">Batcher</span>
                                                </button>
                                            )}
                                            {enabledTools.includes('stock') && (
                                                <button onClick={() => setActiveTool('stock')} className="flex flex-col items-center justify-center p-3 rounded-lg bg-orange-50/50 hover:bg-orange-100 text-orange-700 transition-colors gap-2 hover:scale-105 transform duration-200 border border-orange-100/50">
                                                    <Icon svg={ICONS.box} className="w-5 h-5" /> <span className="text-xs font-medium">Stock</span>
                                                </button>
                                            )}
                                            {enabledTools.includes('trend_locator') && (
                                                <button onClick={() => setActiveTool('trend_locator')} className="flex flex-col items-center justify-center p-3 rounded-lg bg-purple-50/50 hover:bg-purple-100 text-purple-700 transition-colors gap-2 hover:scale-105 transform duration-200 border border-purple-100/50">
                                                    <Icon svg={ICONS.trending} className="w-5 h-5" /> <span className="text-xs font-medium">Trends</span>
                                                </button>
                                            )}
                                            {enabledTools.includes('make_menu') && (
                                                <button onClick={() => setActiveTool('make_menu')} className="flex flex-col items-center justify-center p-3 rounded-lg bg-pink-50/50 hover:bg-pink-100 text-pink-700 transition-colors gap-2 hover:scale-105 transform duration-200 border border-pink-100/50">
                    ...
                    {/* FOOTER */}
                    <div className="p-4 border-t border-slate-100/50 dark:border-slate-800/50 bg-white/80 dark:bg-slate-900/80 backdrop-blur-sm flex justify-end">
                        <Button onClick={onClose} className="bg-slate-900 text-white hover:bg-slate-800 px-8 border border-white/10 shadow-lg">Guardar y Cerrar</Button>
                    </div>
