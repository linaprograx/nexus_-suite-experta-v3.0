            ...
            <AvatarColumn title="Mapa Operativo">
                <div className="space-y-6">
                            ...
                            {['Barra Principal', 'Estación Fría', 'Estación Caliente', 'Almacén'].map((s) => (
                                <button
                                    ...
                                    className={`w-full text-left px-4 py-3 rounded-lg text-sm transition-all border ${selectedStation === s
                                            ? 'bg-rose-500/20 border-rose-500/50 text-rose-200 shadow-[0_0_15px_rgba(99,102,241,0.15)]'
                    ...
                    <div className="bg-slate-800/50 p-4 rounded-xl border border-slate-700/50">
                        ...
                        <select
                            className="w-full bg-slate-900/60 border border-slate-700 rounded-lg p-3 text-slate-300 text-sm outline-none focus:ring-2 focus:ring-rose-500/50"
                    ...
                    <button
                        ...
                        className="w-full py-4 bg-gradient-to-r from-rose-600 to-violet-600 hover:from-rose-500 hover:to-violet-500 text-white font-bold rounded-xl shadow-lg shadow-rose-900/30 transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                    >
                        {isSimulating ? (
                            <>
                    ...
                    <div className="aspect-video bg-slate-900/80 rounded-xl border border-slate-700/50 relative overflow-hidden flex items-center justify-center group">
                        ...
                        <div className="grid grid-cols-6 grid-rows-4 gap-1 w-3/4 h-3/4 opacity-20 group-hover:opacity-40 transition-opacity">
                            {[...Array(24)].map((_, i) => <div key={i} className="border border-rose-400/30 rounded-sm"></div>)}
                        </div>
                        <span className="absolute text-xs font-mono text-rose-400 tracking-widest bg-slate-950/80 px-3 py-1 rounded-full border border-rose-500/30 backdrop-blur-sm">
                ...
                <div className="space-y-6">
                    ...
                    <SectionBlock title="Modo Competición">
                        <div className="grid grid-cols-2 gap-3 mb-3">
                            <div className="p-3 bg-slate-900/50 rounded-lg border border-slate-700 text-center cursor-pointer hover:border-rose-500/50 transition-colors">
                        ...
                        <div className="space-y-2">
                            <div className="flex items-center justify-between p-2 bg-slate-800/40 rounded-lg border border-slate-700/50">
                                ...
                                <div className="w-8 h-4 bg-rose-500/20 rounded-full relative cursor-pointer border border-rose-500/50">
                                    <div className="absolute right-0.5 top-0.5 w-3 h-3 bg-rose-400 rounded-full shadow-sm"></div>
                                </div>
                            </div>
                            <div className="flex items-center justify-between p-2 bg-slate-800/40 rounded-lg border border-slate-700/50">
                    ...
                    <div className="mt-auto pt-4 border-t border-slate-700/50">
                        <button className="w-full py-3 bg-slate-100 hover:bg-white text-slate-900 font-bold rounded-xl shadow-lg shadow-slate-900/20 transition-all flex items-center justify-center gap-2 text-xs">
                            <Icon svg={ICONS.fileText} className="w-4 h-4 text-rose-600" />
...
const SectionBlock = ({ title, children }: { title: string, children?: React.ReactNode }) => (
    <div className="bg-slate-800/30 p-4 rounded-xl border border-slate-700/50 shadow-sm">
        <h4 className="text-rose-400/80 font-bold mb-3 text-[10px] uppercase tracking-wider">{title}</h4>
        {children}
    </div>
);
